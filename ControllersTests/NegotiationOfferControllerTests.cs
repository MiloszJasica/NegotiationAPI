...
{
    public class NegotiationOfferControllerTests
    {
        private readonly ApplicationDbContext _context;
        private readonly NegotiationService _service;

        public NegotiationOfferControllerTests()
        {
        ...
        [Fact]
        public async Task GetNegotiationOffersAsync_ReturnsNegotiationOffers()
        {
            ...
            var okResult = Assert.IsType<OkObjectResult>(result);
            var returnedOffers = Assert.IsType<List<NegotiationOffer>>(okResult.Value);
            Assert.Equal(offers.Count, returnedOffers.Count);
        }
    }
}
